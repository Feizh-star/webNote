...
function genForm(h, formCfg) {
  ...
  listenersList.push({
    ...
    handler: function (event) { formData[key] = event }
  })
  // 添加额外的v-model绑定的事件
  if (inputHandler) {
    listenersList.push({
  ...
  for (let item of customValidators) {
    ...
    listenersList.push({
      ...
      handler: function (...args) {
        item.validator.apply(this, [...args, msg => {
  ...
  let options = []
  if (optionComponent) {
    options = genOptionsVnode ? genOptionsVnode.call(this, h, formCfg, childDepData[key]) : defaultGenOptionsVnode.call(this, h, formCfg, childDepData[key])
  }
  ...
  render(h) {
    ...
    return h(
      ...
      [
        ...
        ...forms.filter(item => !item.hidden).map((item, index) => {
    ...
    validate(cb) {
      ...
      customRulesKeys.forEach(key => {
        ...
        for (let r of ruleArr) {
          r.validator.apply(this, [this.formData[key], this.formData, msg => {
            this.customErrorMsg[key] = msg
          }])
