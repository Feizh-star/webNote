...
export default {
  ...
  computed: {
    editFormsCfg() {
      return this.config
        .filter(item => item.formCfg?.type?.includes('edit'))
        .map(item => ({
          ...item.formCfg,
    ...
    queryFormsCfg() {
      return this.config
        .filter(item => item.formCfg?.type?.includes('query'))
        .map(item => ({
          ...item.formCfg,
    ...
    tableCfg() {
      return this.config.filter(item => item.props || item.formatter)
    }
  },
  watch: {
    /**
  ...
  methods: {
    ...
    editData(row) {
      ...
      this.editFormsCfg.forEach(item => {
    ...
    deleteSelection(selected) {
      this.$confirm('是否确认删除?', '提示', {
        ...
        const ids = selected.map(item => item[this.mainKey]).join(',') || ''
        this.deleteApi(ids).then(res => {
          if (res.code != '200') throw new Error('api-failed')
          this.queryList()
  ...
  render(h) {
    ...
    const mergeEditDialogEvents = {
      'update:visible': val => this.dialogShow = val,
      ...editDialogEvents,
    }
    ...
    return <div class="crud-container">
      ...
      <div class="crud-edit">
        <el-button type="primary" size="small" icon="el-icon-plus" vOn:click={add}>新增</el-button>
        <el-button type="primary" plain size="small" icon="el-icon-delete" vOn:click={() => deleteSelection(selected)} disabled={selected.length === 0}>批量删除</el-button>
        { this.$slots.editBtns }
      </div>
      ...
      >
        ...
        <div slot="footer">
          <el-button type="primary" plain size="small" vOn:click={cancel}>取消</el-button>
          <el-button type="primary" size="small" vOn:click={dialogConfirm}>确定</el-button>
        </div>
      </el-dialog>
    </div>
  },
