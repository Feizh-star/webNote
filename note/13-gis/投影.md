...
<img src=".\projection-img\img1.jpg" style="zoom:45%;" />

![](.\projection-img\img2.jpg)
...
纬线方向的距离在球面上的一段微小弧长为：$$ds_{sphere, p} = R \cos\varphi \cdot d\lambda$$（因为纬度 $\varphi$ 处，经线圈的半径是 $R\cos\varphi$）

在投影面上，对应的微小长度为：$$ds_{map, p} = dx = R \cdot d\lambda$$

因此，纬线方向的比例尺为：
$$
m_p = \frac{ds_{map, p}}{ds_{sphere, p}} = \frac{R \cdot d\lambda}{R \cos\varphi \cdot d\lambda} = \frac{1}{\cos\varphi} = \sec\varphi
$$
...
在球面上，经线方向的微小弧长为：$$ds_{sphere, m} = R \cdot d\varphi$$

因此，经线方向的比例尺为：
$$
m_m = \frac{dy}{R \cdot d\varphi}
$$

**第三步：利用等角性建立微分方程**

根据等角要求 $m_p = m_m$，我们得到：$$\sec\varphi = \frac{dy}{R \cdot d\varphi}$$

整理得到关于 $y$ 的微分方程：
$$
dy = R \cdot \sec\varphi \cdot d\varphi
$$
