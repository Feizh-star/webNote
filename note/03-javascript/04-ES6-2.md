...
1. ES6 为字符串添加了迭代器接口（详见《Iterator》一章），使得字符串可以被`for...of`循环遍历。
...
2. 这个`for...of`迭代器最大的优点是可以识别大于`0xFFFF`的码点
...
7. ES6 提供三个新的方法——`entries()`，`keys()`和`values()`——用于遍历数组。它们都返回一个迭代器对象（详见《Iterator》一章），可以用`for...of`循环进行遍历，唯一的区别是`keys()`是对键名的遍历、`values()`是对键值的遍历，`entries()`是对键值对的遍历。
...
* 可以使用for...of遍历（for...of会使用数据结构的迭代器生成函数，所以只要内置了迭代器的数据结构，都可以使用for...of）
* 可以对Set结构使用...（扩展运算符）
  * 事实上，凡是内置默认迭代器生成函数的数据结构，都可以使用...或for...of
...
4. 遍历方法：
   ...
   * `Set.prototype.keys()`：返回键名的迭代器，由于 Set 结构没有键名，只有键值（或者说键名和键值是同一个值），所以`keys`方法和`values`方法的行为完全一致。下同
   * `Set.prototype.values()`：返回键值的迭代器
   * `Set.prototype.entries()`：返回键值对的迭代器
   * `Set.prototype.forEach()`：使用回调函数遍历每个成员，同数组的forEach方法
   * 由于Set 结构的实例默认可遍历，它的默认迭代器生成函数就是它的`values`方法。所以可以直接用`for...of`循环遍历 Set。
...
4. 遍历方法：
   ...
   * `Map.prototype.entries()`：返回所有成员的迭代器。
     * 由于Map结构的实例默认可遍历，它的默认迭代器生成函数就是它的`entries`方法。所以可以直接用`for...of`循环遍历 Set。
...
* Symbol.iterator：定义在 数据结构对象 或 其原型链 上的一个名为\[Symbol.iterator]()的函数，这个函数是一个迭代器生成函数。

* 1句话概括Iterator：调用1个数据结构的迭代器生成函数，它会返回1个迭代器对象，迭代器对象包含1个指针，指向此数据结构（实例）的起始位置，每调用1次迭代器对象的next()方法，就会返回当前位置的值，并把指针向后移动...直至迭代完成
  
  * 所谓指针，以数组为例，就是数组的索引，指针移动就是把索引增加
  
* 相关文档：

  [Iterator - JavaScript | MDN](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Iterator)

  [迭代协议 - JavaScript | MDN](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Iteration_protocols#迭代器协议)
...
* 由于for...of就是使用迭代器对象来遍历数据结构的，所以只要是部署了Iterator的数据结构，都可以使用for...of遍历（很遗憾，普通对象除外）
...
* 以上4种js内置数据结构，都有keys、values、entries这3个方法，唯有对象的这3个方法返回的是数组，其余三种返回的都是迭代器对象，供for...of使用
...
异步生成器（AsyncGenerator）就是一个next总是返回Promise的迭代器，它总是由异步生成器函数返回（async function*）

[AsyncGenerator - JavaScript | MDN](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/AsyncGenerator)



### 3. async function*

使得生成器函数内部可以写await。返回异步生成器（AsyncGenerator）每次执行next，就执行到下一个yield

* 对于每一个yield块中的多await的理解：多个await会被编译为`Promise.then.then. ... then`，而每次next，正好是要返回Promise的，于是到第一个await时，这个yield就执行完了，第一个await到yield的代码实际上都在Promise微任务中了，调用者只需要`await asyncGenerator.next()`就行了
* 如果yield块全是同步代码，则会被包装成立即兑现的Promise

[async function* - JavaScript | MDN](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/async_function*)



### 4. for await ... of iterable

遍历可迭代对象（`iterable`）的异步迭代器（ [`[Symbol.asyncIterator]`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Symbol/asyncIterator)），如果可迭代对象（`iterable`）的迭代器是同步的（[`[Symbol.iterator]()`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Symbol/iterator) ），则会被包装成异步（将next、return、throw等迭代器方法返回的东西包装一层Promise）

因为每一次的循环体都是一个微任务，所以如果用它来迭代同步可迭代对象，会降低运行速度，一般用于迭代异步生成器（异步迭代器）
