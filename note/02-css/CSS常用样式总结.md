...
当一个div设置了display: flex; flex-direction: column，它有两个子元素，child-1高度由内容撑开或固定高度，child-2设置flex: 1，如果child-2的内容超过了父级的剩余高度，child-2将不再只占据父级的剩余高度，而是被撑开。此时如果设置child-2的min-height为0，则可以解决这个问题

这是一个前端开发中非常经典且让人困惑的问题。简单来说，这涉及到 Flexbox 布局中一个默认的安全机制：**最小内容尺寸（Minimum Content Size）**。

##### 1. 核心原因：`min-height` 的默认值
在普通的块级布局（Block Layout）中，`min-height` 的默认值是 `0`。但在 Flex 布局中，Flex 项目（flex items）的 `min-height` 默认值是 **`auto`**。

当 `min-height` 为 `auto` 时，浏览器会遵循一个基本原则：**内容不应该被强行截断或溢出，除非开发者明确要求这样做。**

##### 2. 为什么 `flex: 1` 没能“拉住”子元素？
你可能会认为 `flex: 1`（等同于 `flex-grow: 1; flex-shrink: 1; flex-basis: 0%;`）应该让子元素强制填满剩余空间并防止溢出。

然而，Flexbox 的规范规定：**Flex 项目的最终尺寸不能小于其“最小内容尺寸”**。
* 如果 `child-2` 内部有一大段文本或一个很高的图片，它的“最小内容尺寸”就是这些内容渲染出来的高度。
* 即便 `flex-shrink` 试图压缩 `child-2` 以适应父容器，它也会在到达 `min-height: auto`（即内容高度）时停止压缩。

结果就是：**内容高度 > 父级剩余高度**，于是 `child-2` 撑开了父容器（或者溢出了父容器，取决于父容器是否有限制高度）。

##### 3. `min-height: 0` 的魔法
当你手动设置 `min-height: 0` 时，你实际上是覆盖了浏览器的默认安全机制：

1.  **解除限制**：你告诉浏览器：“我允许这个元素的高度小于它的内容高度。”
2.  **触发压缩**：此时 `flex-shrink: 1`（包含在 `flex: 1` 中）终于可以生效了。
3.  **精确计算**：浏览器计算出父容器的剩余空间，并将 `child-2` 的高度精确限制在这个范围内，多出的内容会根据你的 `overflow` 设置（如 `scroll` 或 `hidden`）进行处理。

这其实是 Flexbox 为了防止内容在默认情况下被意外遮挡而设计的一个“保护坑”。设置 `min-height: 0` 就像是关闭了“自动扩容”的保险开关，让 `flex-basis` 和 `flex-shrink` 重新拿回了尺寸的控制权。
