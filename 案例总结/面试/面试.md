...
1. 强缓存：都是服务器把以下字段填入http响应头中，告诉浏览器进行资源缓存
   ...
   * 观察：状态码200，size是disk cache或memory cache
...
2. 协商缓存：就是浏览器请求资源时发现它的缓存（可能根本就没有缓存）过期了，通过协商缓存来确定是否需要更新这个资源
   ...
   * 过程：
     ...
     * 服务器根据这两个信息判断此资源是否已过期（更新）
       ...
       * 没有过期，但不完整：服务器响应  206，并下载缺失的部分
       * 过期：服务器响应200，返回新的资源

   * 特殊场景：分段请求，是**协商缓存**

     * 场景：fetch分段加载，可暂停请求，经过暂停后，数据会被分为多段（假如多次暂停），只有最后一段会被缓存，（为了拥有完整缓存）再次fetch这个资源，浏览器会重新加载并缓存前面丢失的部分。至此这个资源就存在多段缓存数据，下次请求时，浏览器不能确定是否有缺失。

     * 此时有多段缓存，下次请求时浏览器会发请求询问服务端这些片段资源是否完整，就是**协商缓存**
       * 不完整：返回206，传输缺失的；
       * 完整：返回304，使用这些缓存片段；（有走网络，断网会失败）
         * 作为对比：一次性传输完毕的资源，被缓存后，（未过期）再次请求，状态码是200，size显示disk cache（其实没有走网络）
